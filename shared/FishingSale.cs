...
namespace BD2Fishing
{
    ...
    public sealed class FishingSalePlan
    {
        ...
        public static bool CanSell(FishingSaleItem fish) => fish!=null && fish.InvenIndex>0 && fish.FishId>0
            && fish.HasFishTable && fish.HasSaleEntry && !fish.IsLocked && (fish.Grade==1 || fish.Grade==2);
        public static FishingSalePlan Build(IEnumerable<FishingSaleItem> inventory)
        {
            ...
            var sale=all.Where(CanSell).OrderBy(f=>f.InvenIndex).ToArray();
            var batch=sale.Take(MaxBatch).ToArray();var ids=new HashSet<long>(batch.Select(f=>f.InvenIndex));
            return new FishingSalePlan{Items=batch,KeepIds=all.Where(f=>!ids.Contains(f.InvenIndex)).Select(f=>f.InvenIndex).ToArray(),Total=all.Length,Sellable=sale.Length,Protected=all.Length-sale.Length};
        }
    }
    public sealed class FishingSaleProgress
    {
        ...
        public void Begin(FishingSalePlan plan,long now)
        {
            ...
            if(plan==null || plan.Items.Length==0 || plan.Items.Any(f=>!FishingSalePlan.CanSell(f)))throw new InvalidOperationException("没有可安全出售的鱼");
            soldIds=plan.Items.Select(f=>f.InvenIndex).ToArray();keepIds=plan.KeepIds.ToArray();sentAt=now;Pending=true;
            Status="正在出售 "+soldIds.Length+" 条普通／稀有鱼，保留传说和锁定鱼";
        }
