...
public sealed class FishingSettings
{
 ...
 public FishingRetentionOptions Retention {get;set;}=new();
...
public sealed class FishingControlLink:IDisposable
{
 ...
 public void Configure(FishingSettings s)
 {
  ...
  lock(sync){command.NextCastMilliseconds=s.NextCastMilliseconds;command.CastGauge=s.CastGauge;command.PreferWeak=s.PreferWeak;command.AutoSell=s.AutoSell;command.Retention=retention;command.AutoBait=s.AutoBait;command.AutoMapRenewal=s.AutoMapRenewal;FishingJson.Write(Path.Combine(root,"settings.json"),s);if(command.Enabled)Write();}
 }
