...
1. 从 Releases 下载 `BD2Fishing-0.2.0.exe`，或者下载包含使用说明和许可证的 ZIP。无需安装 .NET、Python、Visual Studio 或游戏开发 SDK。
...
`build.ps1` 编译桌面程序并执行钓鱼状态机、出售／用饵保护、跨版本解析回归。`package.ps1` 再执行单 EXE 身份与 WPF 界面检查，输出 `dist/` 中的 EXE、ZIP、SHA256 校验文件和发行信息；不会连接游戏。
